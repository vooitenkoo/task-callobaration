...
public class ProjectService {
    ...
    public ProjectResponseDTO createProject(User currentUser, ProjectRequestDTO dto) {
        ...
        // Добавляем ProjectMember для создателя (OWNER/LEAD)
        ...
        // Добавляем остальных участников из DTO (если есть)
        if (dto.members() != null) {
            for (ProjectRequestDTO.ProjectMemberDTO m : dto.members()) {
                // Не добавлять дубликат для создателя
                if (m.userId() != null && !m.userId().equals(currentUser.getId())) {
    ...
    public ProjectResponseDTO updateProject(UUID id, ProjectRequestDTO dto, UUID userId) {
        ...
        // 2. Удаление отсутствующих — больше не делаем удаление участников при обновлении
        return ProjectMapper.toDto(project);
    }
